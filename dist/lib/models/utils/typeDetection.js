...
exports.getTypeDetectionConfidence = getTypeDetectionConfidence;
...
function detectModelType(model) {
    ...
    // Audio models
    if (inputKeys.includes('audio') || inputKeys.includes('speech') || inputKeys.includes('wav') || inputKeys.includes('mp3'))
        return 'audio';
    // Video models  
    if (inputKeys.includes('num_frames') || inputKeys.includes('fps') || inputKeys.includes('init_video') ||
        inputKeys.includes('video') || inputKeys.includes('frames') || inputKeys.includes('duration'))
        return 'video';
    // Text models
    if (inputKeys.includes('max_tokens') || inputKeys.includes('max_new_tokens') || inputKeys.includes('system_prompt') ||
        inputKeys.includes('temperature') || inputKeys.includes('top_p') || inputKeys.includes('messages'))
        return 'text';
    // Image generation models
    if ((inputKeys.includes('width') && inputKeys.includes('height')) ||
        (inputKeys.includes('prompt') && (inputKeys.includes('seed') || inputKeys.includes('guidance_scale'))))
        return 'image';
 ...
 */
function detectFromKnownModels(modelName, owner, description) {
    // Image generation models
 ...
 */
function detectTypeFromDescription(description, modelName) {
 ...
 */
function getTypeDetectionConfidence(model, detectedType) {
    const description = model.description?.toLowerCase() || '';
    const inputSchema = model.latest_version?.openapi_schema?.components?.schemas?.Input?.properties;
    ...
    if (inputSchema) {
        ...
        if (detectedType === 'audio' && (inputKeys.includes('audio') || inputKeys.includes('speech')))
            confidence += 0.4;
        if (detectedType === 'video' && (inputKeys.includes('num_frames') || inputKeys.includes('fps')))
            confidence += 0.4;
        if (detectedType === 'text' && (inputKeys.includes('max_tokens') || inputKeys.includes('system_prompt')))
            confidence += 0.4;
        if (detectedType === 'image' && inputKeys.includes('width') && inputKeys.includes('height'))
            confidence += 0.4;
    }
    ...
    if (model.run_count > 1000)
        confidence += 0.05;
    if (model.run_count > 10000)
        confidence += 0.05;
 ...
 */
function getSuggestedTags(model, detectedType) {
    const description = model.description?.toLowerCase() || '';
    const tags = [];
    // Add type-specific tags
    switch (detectedType) {
        case 'image':
            if (description.includes('flux'))
                tags.push('flux');
            if (description.includes('stable diffusion') || description.includes('sd'))
                tags.push('stable-diffusion');
            if (description.includes('realistic'))
                tags.push('realistic');
            if (description.includes('anime'))
                tags.push('anime');
            if (description.includes('art'))
                tags.push('artistic');
            break;
        case 'video':
            if (description.includes('anime'))
            ...
            break;
        case 'audio':
            if (description.includes('whisper'))
                tags.push('whisper');
            if (description.includes('transcribe'))
                tags.push('transcription');
            if (description.includes('translate'))
                tags.push('translation');
            if (description.includes('music'))
                tags.push('music');
            break;
        case 'text':
            if (description.includes('llama'))
                tags.push('llama');
            if (description.includes('gpt'))
                tags.push('gpt');
            if (description.includes('chat'))
                tags.push('chat');
            if (description.includes('instruct'))
                tags.push('instruction-following');
            break;
        case 'utility':
            if (description.includes('upscale'))
                tags.push('upscaling');
            if (description.includes('background'))
                tags.push('background-removal');
            if (description.includes('detect'))
                tags.push('detection');
            if (description.includes('segment'))
                tags.push('segmentation');
            if (description.includes('caption'))
                tags.push('captioning');
            break;
    }
    // Add general capability tags
    if (description.includes('fast'))
        tags.push('fast');
    if (description.includes('high quality') || description.includes('hq'))
