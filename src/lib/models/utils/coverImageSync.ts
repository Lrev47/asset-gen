...
import { createReplicateAdapter } from '../adapters/ReplicateAdapter'
import prisma from '@/lib/db/prisma'
import type { ReplicateModel } from './typeDetection'

/**
 * Sync cover image URLs for AiModels from Replicate API
 */
export async function syncModelCoverImages(modelSlugs?: string[]): Promise<{ success: boolean; updated: number; errors: string[] }> {
  const errors: string[] = []
  let updated = 0
  ...
  try {
    ...
    for (const model of models) {
      try {
        ...
        const replicateModel = await adapter.getModelInfo(model.slug) as ReplicateModel
 ...
 */
export async function syncSingleModelCoverImage(modelSlug: string): Promise<{ success: boolean; updated: boolean; error?: string }> {
  try {
    const result = await syncModelCoverImages([modelSlug])
    const returnValue: { success: boolean; updated: boolean; error?: string } = { 
 ...
 */
export async function getModelCoverImage(modelSlug: string): Promise<string | null> {
  try {
